...
class Logger_model extends BASE_Model {
    ...
    private function _base_data($log_data = '', $params = '', $method='' ,$api_url) {
        ...
        $api_url = $api_url ? $api_url : CUR_URL;
        $get_param = array();
        $post_param = array();
        if(!empty($params))
        {
        ...
        {
            $get_param = $this->input->get(null,FALSE);
            $post_param =  $this->input->post(null,FALSE);
        }
        $data['trace_id'] = $GLOBALS['_traceId'];
        $data['appkey'] = $this->input->get('appkey') ? $this->input->get('appkey') : '';
